...
function Note(props) {
    ...
    const nContext = useContext(notesContext)
    const { deleteNote, loader } = nContext
    return (
        <div>
                ...
                <CardFooter className="absolute bottom-1">
                    <div className="container flex gap-3">
                        <Tooltip className="bg-black text-white dark:bg-white dark:text-black" content="Delete Note" placement="bottom">
                            <Button color="red" size="sm" onClick={(() => { deleteNote(id); })}>{loader ? <Spinner className='w-4 h-4' /> : <i className="fa-solid fa-trash utils"></i>}</Button>
                        </Tooltip>
